...
class SidrTrigger extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $sidr_theme = $settings->get('sidr_theme') ?: static::DEFAULT_THEME;
    // Wrapper element.
    $build = [
      '#type' => 'container',
      '#theme_wrappers' => [
        'container' => [
          '#attributes' => [
            'class' => ['sidr-trigger', 'js-sidr-trigger'],
            'data-sidr-options' => json_encode($this->getSidrJsOptions()),
          ],
        ],
      ],
      '#attached' => [
